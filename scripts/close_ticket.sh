...
if [ -n "$BRANCH_NAME" ] && [ "$BRANCH_NAME" != "HEAD" ]; then
  ...
  if command -v gh &>/dev/null; then
    ...
    if [ -z "$EXISTING_PR" ]; then
      MERGED_PR=$(gh pr list --head "$BRANCH_NAME" --state merged --json number,url --jq '.[0].number' 2>/dev/null || true)
    fi
  fi

  if [ -n "$MERGED_PR" ]; then
    echo "✅ PR already merged (#${MERGED_PR})"
  elif [ "$HAS_REMOTE" -gt 0 ] && [ -z "$EXISTING_PR" ] && command -v gh &>/dev/null; then
