...
> - `IM`: Fifth Project (`lines-fifth-project-*`)
...
| Metadata | Value |
| :--- | :--- |
| **Status** | Draft |  <!-- enum: Draft, Active, Hold, Done (ticket-level, per RULES Status Definitions). CS-level Review/Staged/verifying → §9 Evidence / CHANGESETS, not ticket Status. -->
| **Created** | YYYY-MM-DD |
| **Owner** | @user |
...
## 4. Implementation Plan (WIP TODO — advisory, not completion evidence)
> Execution scratch. Use plain bullets, NOT `- [ ]` checkboxes. This section is not a completion gate.
> Completion is proven ONLY in §9 Evidence + `tasks/CHANGESETS.md` (commit/PR).
...
> Encode acceptance in §5 Scenarios (BDD): one scenario = one acceptance test.
> Record pass/fail proof in §9 Evidence. Do NOT add `- [ ]` AC items here (they duplicate §5 and drift).
